-- NulaDesign — tek seferde kurulum (PostgreSQL)
-- Sıra: 00 → 01 → 02 içeriği birleşik
-- cPanel'de DB'yi "nuladesign" adıyla oluşturduktan sonra bu dosyayı çalıştır.

-- NOT: CREATE DATABASE yetkin yoksa bu satırı atla, panelden DB oluştur.
-- CREATE DATABASE nuladesign WITH ENCODING 'UTF8' TEMPLATE template0;
...
VALUES ('a0000000-0000-4000-8000-000000000001', 'admin@fashion.local',
    '100000.So8sEZ5VdwO6bR+IRMKQWg==.5esf5PVGXuuMrLv/RFDqmNxRlJYjOQZc1q5tyuC5PH8=',
    'Admin', 'admin', NOW() AT TIME ZONE 'UTC')
ON CONFLICT DO NOTHING;

INSERT INTO "CreditWallets" ("Id", "UserId", "Balance")
VALUES ('b0000000-0000-4000-8000-000000000001', 'a0000000-0000-4000-8000-000000000001', 500)
ON CONFLICT DO NOTHING;

INSERT INTO "CreditTransactions" ("Id", "UserId", "Amount", "Type", "Description", "CreatedAtUtc")
VALUES ('c0000000-0000-4000-8000-000000000001', 'a0000000-0000-4000-8000-000000000001', 500, 1, 'Initial seed credits', NOW() AT TIME ZONE 'UTC')
ON CONFLICT DO NOTHING;
